...
- **VGN Departure Card (`custom:vgn-departure-card` v1.7.5 & `vgn-departure-card-loader.js`)**:
  ...
  - **Unit Test Suite Expansion (`test_vgn_departure_card.js`)**: Added `Timetable & RapidJSON Payload Compatibility Suite` covering `stopEvents` vs `departureList` extraction, numeric vs string transport line normalization, and empty-calendar deduplication contract verification.
