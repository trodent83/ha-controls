...
class MultiPropertyCardEditor extends HAControlThresholdBase {
  ...
  /**
   * Initializes the MultiPropertyCardEditor component instance,
   ...
   * Keeps only multi property card schema fields.
   * 
   * @private
   */
  _cleanConfig() {
    ...
    if (this._config.entities && Array.isArray(this._config.entities)) {
      cleaned.entities = this._config.entities.map(ent => {
        ...
        if (ent.animation !== undefined) e.animation = ent.animation;
        ...
        if (ent.thresholds && Array.isArray(ent.thresholds)) {
          e.thresholds = ent.thresholds.map(thresh => {
            const t = {};
            if (thresh.value !== undefined) t.value = thresh.value;
            if (thresh.color !== undefined) t.color = thresh.color;
            if (thresh.animation !== undefined) t.animation = thresh.animation;
            return t;
          });
        }
  ...
  _resetConfig() {
    this._config = {
      type: this._config?.type || "custom:multi-property-card",
      entities: []
    };
    this._fireConfigChanged();
  }
