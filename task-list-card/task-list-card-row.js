...
class TaskListCardRow extends HAControlBase {
  ...
  /**
   * Dispatches custom 'hold-task' event to parent card container.
   * 
   * @param {Task} task - Task held
   * @private
   */
  _holdTask(task) {
    this.dispatchEvent(new CustomEvent('hold-task', { detail: { task } }));
  }
    ...
    return html`
      ...
      return html`
                <task-list-card-item
                  ...
                  @toggle-task=${(e) => { e.stopPropagation(); this._toggleTask(e.detail.task); }}
                  @hold-task=${(e) => { e.stopPropagation(); this._holdTask(e.detail.task); }}
                ></task-list-card-item>
              `;
    })}
        </div>
